...
contract WOPCrowdsale is  MintedBurnedCrowdsale {
    constructor(
    ...
    function withdrawFundsToUserFromToken(address payable user,uint256 tkAmount) public onlyOwner nonReentrant payable returns(uint256){

        require(tkAmount>0,"WOPCrowdsale: Error tkAmount = 0 ");
        
        uint256 ret= _getWeiAmount( tkAmount);
        ...
        emit FundsWithdrawed(address(user), address(_wallet), ret);
    ...
    function withdrawFundsToUser(address payable user,uint256 ret) public onlyOwner nonReentrant payable{
        require(ret<=getMyether(),"WOPCrowdsale: Error insuficients funds");
        user.transfer(ret);
...
contract WOPCrowdsaleDeployer is Ownable {
        ...
        public
    {
        ...
        WOPCrowdsale _crowdsale = new WOPCrowdsale(
